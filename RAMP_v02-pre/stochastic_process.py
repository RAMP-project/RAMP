...
def Stochastic_Process(j):
    Profile, num_profiles, num_years = Initialise_model()
    ...
    '''
    for year_i in range(num_years): #the whole code is repeated for each year that needs to be considered
            ...
            for Us in User_list: #iterates for each User instance (i.e. for each user class)
                ...
                for i in range(Us.num_users): #iterates for every single user within a User class. Each single user has its own separate randomisation
                    ...
                    for App in Us.App_list: #iterates for all the App types in the given User class
                        ...
                        if year_i < App.y_min: #evaluates if the minimum year for the appliance appearance is met or not
                            if prof_i ==0 and i == 0: #for the first day and user in the class, computes the trend of appliances ownership share for all users of the same kind
                                App.share_trend.append(0)
                            continue
                        else:
                            if prof_i ==0 and i == 0: #for the first day and user in the class, computes the trend of appliances ownership share for all users of the same kind
                                if year_i == App.y_min:
                                    yearly_app_share = App.in_share
                                else:
                                    yearly_app_share = random.uniform(App.share_trend[year_i-1],1) #after threshold year, computes random increase in the share of ownership
                                App.share_trend.append(yearly_app_share)

                            if share_index < yearly_app_share: #evaluates also if the i-th user lies in the share of ownership of the appliance
                                pass
                            else:
                                continue
