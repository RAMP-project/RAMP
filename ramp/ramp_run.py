    ...
    if days is None:
        ...
        if parallel is True:
            Profiles_list = stochastic_process(
                j=j,
                fname=fname,
                num_profiles=len(days),
                day_type=[get_day_type(day) for day in days],
                parallel=parallel,
            )
        else:
            for day in days:
                print("Day", day)
                daily_profiles = stochastic_process(
                    j=j,
                    fname=fname,
                    num_profiles=num_profiles,
...
if __name__ == "__main__":
    for j in input_files_to_run:
        try:
            ...
            print(
                "Input files in .xlsx format not found. Running the default files in .py format."
            )
            run_usecase(j=j)
